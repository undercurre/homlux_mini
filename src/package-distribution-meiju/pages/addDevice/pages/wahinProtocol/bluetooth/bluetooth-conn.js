...
export default class BluetoothConn {
    ...
    wx.createBLEConnection({
      ...
      success: (res) => {
        ...
        wx.onBLEConnectionStateChange(function (res) {
          ...
          if (res.connected == false && that.connSuccess) {
            //主动断开连接时不触发该事件
      ...
      fail: (res) => {
        ...
        //   }
        // })
        if ((res.errCode == 10003 || res.errCode == 10012) && retryLinkBlueNum > 0) {
          //蓝牙失败或超时重试
          retryLinkBlueNum--
          setTimeout(async () => {
            await this.refreshCacheOpenBluetoothAdapter()
  ...
  async createRootKey() {
    ...
    this.rootKey = this.crypto.default.hkdfKeyWithSalt(
      this.advertisData,
      16,
      [],
      Common.string2Uint8Array('midea_bleapp'),
    )
